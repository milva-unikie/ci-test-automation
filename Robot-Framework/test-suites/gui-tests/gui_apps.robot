...
Start and close Google Chrome via GUI on LenovoX1
    ...
    [Tags]            SP-T41  #lenovo-x1 feature is broken (border missing)
    Start app via GUI on LenovoX1   ${CHROME_VM}  chrome  Chrome
...
Start and close PDF Viewer via GUI on LenovoX1
    ...
    Start app via GUI on LenovoX1   ${ZATHURA_VM}  zathura   PDF
...
Start and close Sticky Notes via GUI on LenovoX1
    ...
    [Tags]            SP-T201-2  #lenovo-x1 feature is broken
    Start app via GUI on LenovoX1   ${GUI_VM}  sticky-wrapped   Sticky  #.com.vixalien.sticky-wrapped
...
Start and close Calculator via GUI on LenovoX1
    ...
    Start app via GUI on LenovoX1   ${GUI_VM}  gnome-calculator  Calculator
...
Start and close Bluetooth Settings via GUI on LenovoX1
    ...
    Start app via GUI on LenovoX1   ${GUI_VM}  blueman-manager-wrapped-wrapped  Bluetooth
...
Start and close Ghaf Control Panel via GUI on LenovoX1
    ...
    Start app via GUI on LenovoX1   ${GUI_VM}  ctrl-panel  Control
...
Start and close File Manager via GUI on LenovoX1
    ...
    Start app via GUI on LenovoX1   ${GUI_VM}  pcmanfm  Manager
...
Start and close Falcon AI via GUI on LenovoX1
    ...
    Start app via GUI on LenovoX1   ${GUI_VM}  alpaca-wrapped  Falcon
    Close app via GUI on LenovoX1   ${GUI_VM}  alpaca-wrapped  ./window-close-neg.png
...
Open app menu
    [Documentation]    Check if app menu is is open and if not open it
    Connect to VM if not already connected  gui-vm
    # Searches for app menu magnifying glass to identify if app menu is open
    ${pass_status}  ${coordinates}  Run Keyword And Ignore Error  Locate image  ${GUI_TEMP_DIR}search-neg.png   0.90
    IF    $pass_status=='PASS'
